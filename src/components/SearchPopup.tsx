...
export default function SearchPopup({
  ...
  return (
    <Dialog
      open={isOpen}
      className="fixed z-50 top-52"
      onClose={() => setIsOpen(false)}
    >
      <div className="fixed inset-0 bg-black/30 backdrop-blur-sm" />
      <div className="fixed inset-0 p-4 sm:p-8 md:p-28">
        <Dialog.Panel className="border pb-4 border-zinc-300 mx-auto rounded-lg max-w-3xl bg-white shadow-3xl overflow-hidden">
          <Combobox
            onChange={(loc: any) => {
              window.location = loc;
              setIsOpen(false);
          ...
          >
            <div className="border-b flex items-center mb-4 border-zinc-300">
              <Combobox.Input
                className="w-full text-lg border-none p-8 h-8 focus:ring-0 text-zinc-900 placeholder:text-zinc-400"
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Type to search..."
              />
              <span className="p-1 select-none items-center text-zinc-500 justify-center text-xs font-mono tracking-wide leading-3 pointer-events-none border border-zinc-300 rounded-md mr-8 font-thin">
                      ...
                      {content.content.map((section: any) => (
                        <div>
                          <Combobox.Option
                            ...
                            className={({ active }) =>
                              `${
                                active && "bg-zinc-100 text-zinc-900 rounded-lg"
                              }`
                            }
